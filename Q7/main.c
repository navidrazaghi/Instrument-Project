...
//	float b ;
  ...
  while (1)
  {
	  ...
	  if(HAL_ADC_PollForConversion(&hadc1,wait_time)==HAL_OK)
	  {
		  ...
		  float temperature = voltage*16.0 - 20.0 ;
	        int temperatureInt = temperature * 100;
	        int temperatureWhole = temperatureInt / 100;
	        int temperatureFraction = temperatureInt % 100;

	        sprintf(stra, "%d.%02d C", temperatureWhole, temperatureFraction);
		  Lcd_cursor(&lcd, 0, 0);
		  Lcd_clear(&lcd);
		  Lcd_string(&lcd, stra);
	  }
